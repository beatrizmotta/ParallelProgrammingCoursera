...
public class ReciprocalArraySumTest extends TestCase {
    ...
    private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
        ...
        if (useManyTaskVersion) {
            sum = ForkJoinReciprocalArraySum.parManyTaskArraySum(input, ntasks);
        } else {
            assert ntasks == 2;
            sum = ForkJoinReciprocalArraySum.parArraySum(input);
        }
        ...
        for (int r = 0; r < REPEATS; r++) {
            if (useManyTaskVersion) {
                ForkJoinReciprocalArraySum.parManyTaskArraySum(input, ntasks);
            } else {
                assert ntasks == 2;
                ForkJoinReciprocalArraySum.parArraySum(input);
            }
        }
